<?php
  ...
  if (isset($_SESSION['ID'])&&isset($_SESSION['Name'])&&isset($_SESSION['email'])&&$_SESSION['user']===$doctor) {
    ...
    $sql = "SELECT `patientID` FROM `patientappointments` WHERE `docID` = '$id';";
...
?>
    ...
    <html lang="en">
      <head>
        ...
        <title>Doctor · Patients || E.H.S</title>
          ...
          <tbody>
            <?php
              ...
              foreach ($result as $r) {
                ...
                $sql = "SELECT `patientID`, `patientName`, `age`, `DoB`, `gender`, `phoneNo`, `email` FROM `patient` WHERE `patientID`='$patientID';";
